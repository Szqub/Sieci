...
export function HistoryPage({ sessions, selected, busy, error, onRefresh, onSelect, onRestore, onRestoreTargets, onDownloadBundle, onReconcileExternal }: HistoryPageProps) {
  ...
  const filtered = useMemo(() => sessions.filter((session) => {
    ...
    const text = `${session.id} ${session.description} ${session.operator} ${session.panoramaHost} ${session.affectedDeviceGroups.join(" ")} ${(session.targets ?? []).join(" ")} ${(session.backupItems ?? []).map((item) => `${item.entityName} ${item.entityType} ${(item.targets ?? []).join(" ")}`).join(" ")}`.toLowerCase();
  ...
  return <div className="page-stack history-design-page">
    <PageHeader eyebrow="Operations / Backups" title="Historia, backup i Restore" description="Sesje i backupy są trwale zapisane w Dokumenty / PanOS Toolbox. Wyszukaj obiekt, politykę, IP albo session ID i wybierz ostatnią stabilną sesję." actions={<Button onClick={onRefresh} loading={busy} icon={<RefreshCw size={16} />}>Odśwież</Button>} />
    ...
    <div className="history-layout">
      <Card className="history-list-card">
        <div className="history-toolbar"><div className="table-search"><Search size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Szybko: obiekt, polityka, IP, DG, session ID…" /></div><div className="segmented-control"><button className={kind === "all" ? "is-active" : ""} onClick={() => setKind("all")}>Wszystkie</button><button className={kind === "cleanup" ? "is-active" : ""} onClick={() => setKind("cleanup")}>Cleanup</button><button className={kind === "restore" ? "is-active" : ""} onClick={() => setKind("restore")}>Restore</button></div></div>
      ...
      <Card className="session-detail-card">
        {!selected ? <EmptyState icon={<Activity size={27} />} title="Wybierz sesję" description="Domyślnie zaznaczana jest ostatnia stabilna sesja cleanup." /> : <>
          ...
          <div className="persistent-backup-note"><ShieldCheck size={17} /><span><strong>Backup nie znika po zamknięciu.</strong><small>Dokumenty\PanOS Toolbox\sessions\{selected.id}</small></span></div>
